...
/**
 * Test Resources reifier.
 */
public class ResourcesReifierTest {
    ...
    public void testReify() throws Exception {
        ...
        Path target = PROJECT_BASE_DIR.resolve("target/test-data/resourcesA/mapA");
        ProfilesHelpers.deleteDirectory(target);
        Files.createDirectories(target);

        Path repository = REPOSITORIES_BASE_DIR.resolve("resourcesA/profiles");
        final Path materialized = PROJECT_BASE_DIR.resolve("target/test-data/resourcesA-materialized");
        ProfilesHelpers.deleteDirectory(materialized);
        Files.createDirectories(materialized);

        final Path containerConfig = REPOSITORIES_BASE_DIR.resolve("resourcesA/configs/containers/mapA.yaml");
        ...
        mavenConfigDTO.setDescription("Fabric8 Resources mapA container");
        containerConfigDTO = new ContainerConfigDTO();
        containerConfigDTO.setName("mapA");
        ...
        final VelocityBasedReifier reifier = new ResourcesProjectReifier(null);
